...
<!-- TODO: hero GIF — browsing a Seek listing → click "+ Track job" → toast → cut to Trello, card in "Saved" -->
...
So I described it to Claude. This repo is the result: a Chrome extension that puts a **Track job**
...
- **One-click saving** — a button appears on every Seek and LinkedIn job listing; clicking it
...
- **Track jobs from your phone too** — an optional iOS Shortcut adds *Track job* to the share sheet
  inside the LinkedIn and Seek apps, feeding the same board with the same dedup
- **No duplicates, no babysitting** — save the same job twice, even from different devices, and
...
**Act 1: the button.** I described the tool I wanted to Claude — a save button on every job
...
tracker couldn't follow me there.

So it grew a second entry point: an **iPhone Shortcut** in the share sheet. Tap *Share* on any
job in the LinkedIn or Seek app → *Track job* → done. Behind it sits a tiny **Cloudflare Worker**
(free tier) that takes the shared URL, fetches the page, extracts the same fields server-side,
and files the card to the same board — with the same dedup, so a job saved on the phone and
again at the desk shows up once, not twice.

<!-- TODO: short screen recording — share sheet on phone → Track job → Trello card appears -->
...
1. [Download the repo as a zip](https://github.com/kevinryan-au/job-tracker/archive/refs/heads/main.zip)
   and unzip it (or grab a [release](https://github.com/kevinryan-au/job-tracker/releases), or clone)
...
3. Open any Seek or LinkedIn job listing (reload the tab if it was already open) and save —
   you're already tracking, locally
4. *(Optional, a few minutes)* Click the Job Tracker icon → **⚙ Settings** → connect Trello.
   ...
   the **Get my token** button). Paste them, hit **Connect & create my board**, and every saved job
...
opening the repo and saying *"set up mobile saving"* walks you through the whole thing.
...
Recipes saved to Notion. Properties to a spreadsheet. Papers to Zotero. Leads to a CRM. The
